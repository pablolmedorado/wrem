...
import { IParticipante, Participante } from '../../../objetos/shared/models/participante.model';
...
@Component({
  ...
  exportAs: 'ParticipanteForm'
})
export class ParticipanteFormComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('ParticipanteForm') form;
  @Output() onSubmittedForm = new EventEmitter();

  private max_num: number;
  private organizacionesOptions: IObjeto[];
  ...
  constructor(
    ...
    this.objetosSubscription = this.objetoStreamService.objetosMap$.subscribe(map => {
      this.objetos = map;
      this.organizacionesOptions = map.filter(obj => obj.tipo_objeto === TIPOS.ORGANIZACION).toArray();
    });
  }
    ...
    if(this.objetoId){
      this.objetoService.getObjeto(this.objetoId, {expand: 'logs'})
        .then(objeto => {
          ...
          this.formGroup.controls['organizacion'].setValue(this.objeto.organizacion);
          this.formGroup.controls['rol'].setValue(this.objeto.rol);
          this.formGroup.controls['es_cliente'].setValue(this.objeto.es_cliente);
          this.formGroup.controls['es_desarrollador'].setValue(this.objeto.es_desarrollador);
          this.formGroup.controls['es_usuario'].setValue(this.objeto.es_usuario);
        });
    }else{
      let max_obj = this.objetos.filter(obj => obj.tipo_objeto === TIPOS.PARTICIPANTE).max(objetoNumeroComparator);
      let num: number = max_obj ? max_obj.numero + 1 : 1;
      let codigo: string = `${TIPOS.CONFIG[TIPOS.PARTICIPANTE].prefix}-${paddy(num, 4)}`;
      this.objeto = new Participante({documento: this.documentoId, codigo: codigo, numero: num});
    }
  }
  ...
  onSubmit({ value, valid, dirty }: { value: IParticipante, valid: boolean, dirty: boolean }): void {
    this.submitted = true;
    if(!valid){
      ...
      if (dirty){
        ...
        this.objetoService.createOrUpdateObjeto(value, TIPOS.CONFIG[TIPOS.PARTICIPANTE].api_reference)
          .then(obj => {
            this.objeto = obj;
