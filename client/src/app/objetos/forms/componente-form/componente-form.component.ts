...
import { IComponente, Componente, ITipoObjeto } from '../../../objetos/shared/models/tipo-objeto.model';
...
@Component({
  ...
  styleUrls: ['./componente-form.component.css'],
  exportAs: 'ComponenteForm'
})
export class ComponenteFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.formGroup = this.fb.group({
      id: null,
      tipo_objeto: [this.objetoPadreId, Validators.required],
    ...
    if(this.objetoId){
      this.objetoService.getElement(this.objetoId, TIPOS.CONFIG[TIPOS.COMPONENTE].api_reference)
        .then(objeto => {
          this.objeto = objeto;
      ...
      this.objeto = new Componente({tipo_objeto: this.objetoPadreId});
    }
  }
  ...
  onSubmit({ value, valid, dirty }: { value: IComponente, valid: boolean, dirty: boolean }): void {
    this.submitted = true;
    if(!valid){
      this.growlMessagesService.newMessage({severity:'warn', summary: 'Atención', detail:'El formulario contiene errores.'});
    } else {
      if (dirty){
        this.objetoService.createOrUpdateObjeto(value, TIPOS.CONFIG[TIPOS.COMPONENTE].api_reference)
          .then(obj => {
            this.objeto = obj;
            ...
            this.objetoStreamService.updateMap(this.objetos.withMutations(map => {
              let padre: ITipoObjeto = map.get(this.objetoPadreId);
              let hijo_index = padre[TIPOS.CONFIG[TIPOS.COMPONENTE].related_name].findIndex(dat => obj.id === dat.id);

              if(hijo_index > -1){
                padre[TIPOS.CONFIG[TIPOS.COMPONENTE].related_name].splice(hijo_index, 1, obj);
              } else {
                padre[TIPOS.CONFIG[TIPOS.COMPONENTE].related_name].push(obj);
              }
