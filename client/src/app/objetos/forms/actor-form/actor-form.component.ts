...
import { IActor, Actor } from '../../../objetos/shared/models/actor.model';
...
export class ActorFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if(this.objetoId){
      ...
      let max_obj = this.objetos.filter(obj => obj.tipo_objeto === TIPOS.ACTOR).max(objetoNumeroComparator);
      let num: number = max_obj ? max_obj.numero + 1 : 1;
      let codigo: string = `${TIPOS.CONFIG[TIPOS.ACTOR].prefix}-${paddy(num, 4)}`;
      this.objeto = new Actor({documento: this.documentoId, codigo: codigo, numero: num});
    }
  }
  ...
  onSubmit({ value, valid, dirty }: { value: IActor, valid: boolean, dirty: boolean }): void {
    this.submitted = true;
    if(!valid){
      ...
      if (dirty){
        ...
        this.objetoService.createOrUpdateObjeto(value, TIPOS.CONFIG[TIPOS.ACTOR].api_reference)
          .then(obj => {
            this.objeto = obj;
