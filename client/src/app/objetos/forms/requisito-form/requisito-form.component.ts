...
import { IRequisito, Requisito } from '../../../objetos/shared/models/requisito.model';
...
@Component({
  ...
  exportAs: 'RequisitoForm'
})
export class RequisitoFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if(this.objetoId){
      ...
      let max_obj = this.objetos.filter(obj => obj.tipo_objeto === TIPOS.REQUISITO && obj.tipo === this.tipo).max(objetoNumeroComparator);
      let num: number = max_obj ? max_obj.numero + 1 : 1;
      let codigo: string = `${TIPOS.CONFIG[TIPOS.REQUISITO].prefix[this.tipo]}-${paddy(num, 4)}`;
      this.objeto = new Requisito({documento: this.documentoId, codigo: codigo, numero: num, tipo: this.tipo});
    }
  }
  ...
  onSubmit({ value, valid, dirty }: { value: IRequisito, valid: boolean, dirty: boolean }): void {
    this.submitted = true;
    if(!valid){
      ...
      if (dirty){
        ...
        this.objetoService.createOrUpdateObjeto(value, TIPOS.CONFIG[TIPOS.REQUISITO].api_reference)
          .then(obj => {
            this.objeto = obj;
