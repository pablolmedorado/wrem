...
import { ISeccion, Seccion } from '../../../objetos/shared/models/seccion.model';
...
export class SeccionFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if(this.objetoId){
      ...
      let max_obj = this.objetos.filter(obj => obj.tipo_objeto === TIPOS.SECCION).max(objetoNumeroComparator);
      let num: number = max_obj ? max_obj.numero + 1 : 1;
      let codigo: string = `${TIPOS.CONFIG[TIPOS.SECCION].prefix}-${paddy(num, 4)}`;
      this.objeto = new Seccion({documento: this.documentoId, codigo: codigo, numero: num});
    }
  }
  ...
  onSubmit({ value, valid, dirty }: { value: ISeccion, valid: boolean, dirty: boolean }): void {
    this.submitted = true;
    if(!valid){
      ...
      if (dirty){
        ...
        this.objetoService.createOrUpdateObjeto(value, TIPOS.CONFIG[TIPOS.SECCION].api_reference)
          .then(obj => {
            this.objeto = obj;
