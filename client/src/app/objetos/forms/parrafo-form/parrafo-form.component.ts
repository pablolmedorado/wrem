...
import { IParrafo, Parrafo } from '../../../objetos/shared/models/parrafo.model';
...
@Component({
  selector: 'app-parrafo-form',
  templateUrl: './parrafo-form.component.html',
  styleUrls: ['./parrafo-form.component.css'],
  exportAs: 'ParrafoForm'
})
export class ParrafoFormComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('ParrafoForm') form: NgForm;
  @Output() onSubmittedForm = new EventEmitter();

  // TinyMCE
  @ViewChild('TinyMCEEditor') tinyMCEEditor;
  ...
  ngOnInit() {
    ...
    if(this.objetoId){
      ...
      let max_obj = this.objetos.filter(obj => obj.tipo_objeto === TIPOS.PARRAFO).max(objetoNumeroComparator);
      let num: number = max_obj ? max_obj.numero + 1 : 1;
      let codigo: string = `${TIPOS.CONFIG[TIPOS.PARRAFO].prefix}-${paddy(num, 4)}`;
      this.objeto = new Parrafo({documento: this.documentoId, codigo: codigo, numero: num});
    }
  }
  ...
  onSubmit({ value, valid }: { value: IParrafo, valid: boolean }): void {
    this.submitted = true;
    if(valid){
      if(value['fecha_version']){
        value['fecha_version'] = moment((<any>value['fecha_version']).jsdate).format('YYYY-MM-DD');
      }
      value['texto'] = this.tinyMCEEditor.editor.getContent();
      this.objetoService.createOrUpdateObjeto(value, TIPOS.CONFIG[TIPOS.PARRAFO].api_reference)
        .then(obj => {
          this.objeto = obj;
